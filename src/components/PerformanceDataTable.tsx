...
function DimensionSelector({ dimension, onChange }: { dimension: StratificationDimension; onChange: (d: StratificationDimension) => void }) {
  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] uppercase tracking-[0.08em] text-gray-500 font-semibold">Stratify by</span>
      <div className="flex rounded-lg bg-black/[0.03] border border-(--color-border) p-0.5">
        {STRATIFICATION_DIMENSIONS.map(dim => (
          <button
            ...
            className={`rounded-sm px-2.5 py-1 text-[10px] font-medium transition-colors cursor-pointer ${
              dimension === dim.id
                ? 'bg-white text-gray-900 border border-(--color-border-subtle)'
          ...
          <table className="w-full text-[11px]">
            <thead>
              <tr className="border-b border-(--color-border) bg-black/[0.02]">
                <th className="text-left px-5 py-2.5 text-[10px] uppercase tracking-[0.08em] text-gray-500 font-semibold">
                  {dimension === 'overall' ? 'Segment' : STRATIFICATION_DIMENSIONS.find(d => d.id === dimension)?.label}
                </th>
                <th className="text-right px-3 py-2.5 text-[10px] uppercase tracking-[0.08em] text-gray-500 font-semibold">Count</th>
                {METRIC_COLS.map(col => (
                  <th key={col.key} className="text-right px-3 py-2.5 text-[10px] uppercase tracking-[0.08em] text-gray-500 font-semibold">
                    {col.label}
                  </th>
                ))}
              </tr>
  ...
  return (
    <>
      ...
      <table className="w-full text-[11px]">
        <thead>
          <tr className="border-b border-(--color-border) bg-black/[0.02]">
            <th className="text-left px-5 py-2 text-[10px] uppercase tracking-[0.08em] text-gray-500 font-semibold">
              {dimension === 'overall' ? '' : STRATIFICATION_DIMENSIONS.find(d => d.id === dimension)?.label}
            </th>
            <th className="text-right px-3 py-2 text-[10px] uppercase tracking-[0.08em] text-gray-500 font-semibold">Count</th>
            {METRIC_COLS.map(col => (
              <th key={col.key} className="text-right px-3 py-2 text-[10px] uppercase tracking-[0.08em] text-gray-500 font-semibold">
                {col.label}
              </th>
            ))}
          </tr>
...
export function PerformanceDataTable({ data: stratifiedData, inTabContainer }: Props) {
  ...
  return (
    <div className="rounded-md border border-(--color-border) bg-(--color-surface) overflow-hidden panel-shadow">
      ...
      >
        <span className="text-[10px] uppercase tracking-[0.08em] text-gray-600 font-semibold">Performance Data</span>
